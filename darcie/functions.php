...
if ( ! function_exists( 'darcie_fonts_url' ) ) :
	...
	function darcie_fonts_url() {
		...
		*/
		...
		/* Translators: If there are characters in your language that are not
		* supported by PT Serifs, translate this to 'off'. Do not translate
		* into your own language.
		*/
		$pt_serif = _x('on', 'PT Serif: on or off', 'darcie');

		if ('off' !== $poppins || 'off' !== $pt_serif) {
			$font_families = array();
			...
			if ('off' !== $pt_serif) {
				$font_families[] = 'PT Serifs::300,400,600,700';
			}
